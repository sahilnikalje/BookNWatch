...
const Dashboard = () => {
  ...
  const safeActiveShows = (dashboardData.activeShows || []).filter(
    (show) => show?.movie,
  );
  ...
  const dashboarCards = [
    ...
    {
      title: "Active Shows",
      value: safeActiveShows.length || "0",
      icon: PlayCircleIcon,
    },
    {
      title: "Total Users",
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      toast.error(
        `Error fetching dashboard data: ${err?.message || "Unknown error"}`,
      );
    } finally {
      setLoading(false);
    }
